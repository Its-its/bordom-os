// Stolen. https://github.com/theseus-os/Theseus/blob/theseus_main/kernel/ps2/src/lib.rs
...
pub fn status_register() -> ControllerToHostStatus {
    ControllerToHostStatus::from_bytes([unsafe { PS2_COMMAND_AND_STATUS_PORT.lock().read() }])
}
...
fn read_data() -> u8 {
    unsafe { PS2_DATA_PORT.lock().read() }
}
